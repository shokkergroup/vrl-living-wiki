(function () {
  ...
  function podiumProofCards(result, fallbackSourceId) {
    return (result && result.podium || []).map(function (name, index) {
      ...
      var imageMarkup = driver && driver.image
        ? '<img src="' + esc(driver.image.file) + '" alt="HLRN source frame connected to ' + esc(name) + '">'
  ...
  function centralIssue(id) {
    ...
    app.innerHTML = '<article class="central-issue paper-edition"><header class="paper-mast"><div class="wrap"><div><small>THE OFFICIAL RACE PAPER OF THE HIGH LINE</small><h1>HIGHLINE <i>CENTRAL</i></h1></div><section><b>SEASON ' + issue.season + " / EDITION " + String(issue.race).padStart(2, "0") + '</b><time>' + esc(fmtDate(issue.date).toUpperCase()) + '</time><span>' + issue.wordCount + ' EDITORIAL WORDS</span></section></div></header>' +
      ...
      (issue.companion ? '<section class="issue-companion"><span>CONNECTED COMPANION</span><img src="' + esc(issue.companion.thumb) + '" alt=""><h3>' + esc(issue.companion.title) + '</h3><button onclick="__play(\'' + issue.companion.id + '\',0,' + playArg(issue.companion.title) + ')">▶ PLAY THE SHOW</button></section>' : '') +
      "</aside></div></article>";
  }
  ...
  function driversPage() {
    ...
    app.innerHTML = '<div class="drivers-page">' + pageHead("FRONTLINE GARAGE / DRIVER DOSSIERS", "THE CARS.<br><em>THE RESULTS.</em><br>THE STORIES.", "Source-backed car and race frames lead the garage. Every dossier then connects result receipts, Central coverage, signature tape, track history, and the broader appearance index.", [
      ...
      '<section class="driver-evidence-key"><header><span>THE DOSSIER LADDER</span><h2>FILTER BY WHAT THE ARCHIVE CAN ACTUALLY PROVE</h2></header><div>' + tierOptions.map(function (option) { var count = option[0] === "all" ? DATA.drivers.length : DATA.drivers.filter(function (driver) { return (driver.evidenceTier || {}).id === option[0]; }).length; return '<button class="' + (state.driverTier === option[0] ? "on" : "") + '" onclick="__driverTier(\'' + option[0] + '\')"><span>' + option[1] + '</span><b>' + count + '</b></button>'; }).join("") + '</div><p>Result-backed means a recovered top-three position. Official tape means recurring presence only. Highline Live never becomes an official HLRN career by proximity.</p></section>' +
      '<section class="garage-register"><header><div><span>THE COMPLETE IDENTITY REGISTER</span><h2>' + (query || state.driverTier !== "all" ? "FILTERED DOSSIERS" : "EVERY NORMALIZED DRIVER") + '</h2></div><div class="driver-search"><span>FIND A GARAGE PASS</span><input value="' + esc(state.driverQuery) + '" placeholder="Driver or team…" oninput="__driverFilter(this.value)"><b>' + drivers.length + " MATCHES</b></div></header><div class=\"driver-grid\">" + (drivers.length ? drivers.map(driverCard).join("") : '<div class="empty-state">No driver file matches this evidence tier and search.</div>') + "</div></section>" +
  ...
  function photoDeskPage() {
    ...
    app.innerHTML = '<div class="photo-desk-page">' + pageHead("BROADCAST CONTACT SHEET / SOURCE-ATTRIBUTED IMAGERY", "THE PHOTO<br><em>DESK.</em>", "Central fronts and driver dossiers gathered into one visual index. Every frame can reopen the exact HLRN source second.", [
      ...
      return '<figure><img loading="lazy" src="' + esc(issue.image.file) + '" alt=""><button onclick="__play(\'' + esc(issue.image.sourceId) + '\',' + Number(issue.image.t || 0) + ',' + playArg(issue.headline) + ')">▶ ' + fmtTime(issue.image.t || 0) + '</button><figcaption><b>' + esc(issue.headline) + '</b><a href="#/central/' + issue.id + '">S' + issue.season + ' / R' + issue.race + '</a></figcaption></figure>';
      ...
      return '<figure><img loading="lazy" src="' + esc(driver.image.file) + '" alt=""><button onclick="__play(\'' + esc(driver.image.sourceId) + '\',' + Number(driver.image.t || 0) + ',' + playArg(driver.name) + ')">▶ ' + fmtTime(driver.image.t || 0) + '</button><figcaption><b>' + esc(driver.name) + '</b><a href="#/driver/' + driver.id + '">DOSSIER</a></figcaption></figure>';
    }).join("") + '</div></section></div></div>';
  }
  ...
  function racePage(id, timestamp) {
    ...
    app.innerHTML = '<article class="race-page deep-dive"><section class="race-hero"><div class="race-hero-bg" style="background-image:url(\'' + esc(heroImage) + '\')"></div><div class="wrap"><div class="race-crumb"><a href="' + (source.lane === "official" ? "#/season/" + source.season : "#/highline-live") + '">' + esc(laneLabel(source.lane)) + "</a><span>/</span>" + esc(source.name) + "</div><div class=\"race-title\">" + laneBadge(source) + '<span class="race-file-label">' + (source.lane === "official" ? "OFFICIAL RACE DEEP DIVE" : "HIGHLINE LIVE SOURCE FILE") + '</span><h1>' + esc(issue ? issue.headline : source.name) + "</h1><p>" + esc(source.track) + " · " + esc(fmtDate(source.date)) + " · " + fmtDuration(source.duration) + '</p><div>' + primaryPlay + '<a class="button glass" href="' + esc(source.url) + '" target="_blank" rel="noopener">YOUTUBE SOURCE ↗</a>' + (issue ? '<a class="button glass" href="#/central/' + source.id + '">READ CENTRAL EDITION</a>' : '') + '<button class="button glass" onclick="__shareRace(\'' + source.id + '\')">SHARE FILE</button><button class="button glass" onclick="__downloadRacePack(\'' + source.id + '\')">SOURCE PACK ↓</button></div></div><aside>' + heatBar(source) + '<div><b>' + chapters.length + "</b><span>RACE CHAPTERS</span></div><div><b>" + source.transcriptLines.toLocaleString() + "</b><span>TIMED SEGMENTS</span></div></aside></div></section>" +
      ...
      (source.companion ? '<section class="race-companion"><span>THE SHOW / CONNECTED SOURCE</span><img src="' + esc(source.companion.thumb) + '" alt=""><h3>' + esc(source.companion.title) + '</h3><p>HLRN-authored context and entertainment, separated from the primary scoring lane.</p><button onclick="__play(\'' + source.companion.id + '\',0,' + playArg(source.companion.title) + ')">▶ PLAY THE SHOW</button>' + (issue ? '<a href="#/central/' + source.id + '">READ CENTRAL EDITION →</a>' : '') + '</section>' : "") +
