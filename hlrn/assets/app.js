(function () {
  ...
  function photoDeskPage() {
    ...
    app.innerHTML = '<div class="photo-desk-page">' + pageHead("BROADCAST CONTACT SHEET / SOURCE-ATTRIBUTED IMAGERY", "THE PHOTO<br><em>DESK.</em>", "Central fronts and driver dossiers gathered into one visual index. Every frame can reopen the exact HLRN source second.", [
      ...
      return '<figure><img loading="lazy" src="' + esc(issue.image.file) + '" alt="HLRN Central source frame for ' + esc(issue.headline) + '"><button onclick="__play(\'' + esc(issue.image.sourceId) + '\',' + Number(issue.image.t || 0) + ',' + playArg(issue.headline) + ')">▶ ' + fmtTime(issue.image.t || 0) + '</button><figcaption><b>' + esc(issue.headline) + '</b><a href="#/central/' + issue.id + '">S' + issue.season + ' / R' + issue.race + '</a></figcaption></figure>';
      ...
      return '<figure><img loading="lazy" src="' + esc(driver.image.file) + '" alt="HLRN source frame connected to ' + esc(driver.name) + '"><button onclick="__play(\'' + esc(driver.image.sourceId) + '\',' + Number(driver.image.t || 0) + ',' + playArg(driver.name) + ')">▶ ' + fmtTime(driver.image.t || 0) + '</button><figcaption><b>' + esc(driver.name) + '</b><a href="#/driver/' + driver.id + '">DOSSIER</a></figcaption></figure>';
    }).join("") + '</div></section></div></div>';
  }
